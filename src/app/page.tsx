  ...
  return (
    ...
    >
      <div style={{position:'relative',overflow:'hidden',marginBottom:'10px',aspectRatio:'3/4',background:'#f5f5f5'}}>
        ...
        <button
          style={{position:'absolute',top:'10px',right:'10px',width:'30px',height:'30px',borderRadius:'50%',background:'rgba(255,255,255,0.9)',border:'none',cursor:'pointer',fontSize:'13px',display:'flex',alignItems:'center',justifyContent:'center'}}
...
export default function Home() {
  ...
  return (
    <main>
      ...
      {/* 메인 배너 */}
      <div style={{position:'relative',height:'560px',overflow:'hidden',background:'#1a1814'}}>
        <img
          src={b.image}
          alt="banner"
          style={{position:'absolute',right:0,top:0,width:'55%',height:'100%',objectFit:'cover',opacity:0.4}}
        />
        <div style={{position:'absolute',inset:0,background:'linear-gradient(90deg,#1a1814 40%,transparent 100%)'}}/>
        <div style={{position:'relative',zIndex:1,padding:'0 80px',height:'100%',display:'flex',alignItems:'center'}}>
          <div>
            <div style={{fontSize:'10px',letterSpacing:'4px',color:'#C94E1A',marginBottom:'20px',display:'flex',alignItems:'center',gap:'10px'}}>
              <span style={{display:'inline-block',width:'20px',height:'1px',background:'#C94E1A'}}/>
              {b.tag}
            </div>
            <h1 style={{fontFamily:'Georgia,serif',fontSize:'52px',fontWeight:300,color:'#fff',lineHeight:1.15,marginBottom:'16px',whiteSpace:'pre-line'}}>
              {b.title}
            </h1>
            <p style={{fontSize:'13px',color:'rgba(255,255,255,0.5)',fontWeight:300,lineHeight:1.8,marginBottom:'32px',whiteSpace:'pre-line'}}>
              {b.sub}
            </p>
            <div style={{display:'flex',gap:'12px'}}>
              <a href="/" style={{padding:'13px 28px',background:'#fff',color:'#111',fontSize:'12px',fontWeight:500,letterSpacing:'1px',textDecoration:'none'}}>
                선피팅 신청하기
              </a>
              <a href="/" style={{padding:'12px 24px',border:'1px solid rgba(255,255,255,0.3)',color:'rgba(255,255,255,0.7)',fontSize:'12px',textDecoration:'none'}}>
                전체 상품 보기
              </a>
            </div>
          </div>
      ...
      {/* 퀵메뉴 */}
      <div style={{padding:'28px 40px',borderBottom:'1px solid #e8e8e8',display:'flex',gap:'0',overflowX:'auto'}}>
        {[
          {icon:'📦',label:'선피팅관'},
          {icon:'✨',label:'신상품'},
          {icon:'🔥',label:'베스트'},
          ...
          <div key={item.label} style={{flex:'1',minWidth:'72px',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px',cursor:'pointer',padding:'0 8px'}}>
      ...
      {/* 피팅 서비스 스트립 */}
      <div style={{background:'linear-gradient(90deg,#1a1814,#2c2218,#1a1814)',padding:'18px 40px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',gap:'40px',alignItems:'center'}}>
          ...
          <a href="/" style={{padding:'10px 20px',border:'1px solid rgba(255,255,255,0.25)',color:'rgba(255,255,255,0.8)',fontSize:'12px',textDecoration:'none'}}>신청하기 →</a>
        </div>
      </div>
      ...
      {/* AI 추천 배너 */}
      <div style={{margin:'0 40px',background:'linear-gradient(110deg,#1a1814,#2d2318)',padding:'32px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'40px'}}>
        <div>
        ...
        <a href="/" style={{padding:'12px 24px',background:'#C94E1A',color:'#fff',fontSize:'12px',fontWeight:500,textDecoration:'none',whiteSpace:'nowrap',flexShrink:0}}>AI 취향 분석 시작</a>
      </div>
      ...
      {/* 브랜드 피처 */}
      <div style={{padding:'0 40px 56px',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px'}}>
        {[
          ...
          <div key={item.brand} style={{position:'relative',aspectRatio:'3/4',overflow:'hidden',cursor:'pointer'}}>
            ...
            <div style={{position:'absolute',top:'12px',right:'12px',background:'#C94E1A',color:'#fff',fontSize:'9px',fontWeight:700,padding:'4px 8px',letterSpacing:'0.5px'}}>
              {item.badge}
            </div>
      ...
      {/* Withy 안내 */}
      <div style={{margin:'0 40px 56px',background:'#f5f5f5',border:'1px solid #e8e8e8',padding:'28px 36px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'32px'}}>
        ...
        <div style={{display:'flex',gap:'32px',alignItems:'center',flexShrink:0}}>
          ...
          <a href="/" style={{padding:'12px 22px',background:'#111',color:'#fff',fontSize:'12px',fontWeight:500,textDecoration:'none'}}>위디 사용하기</a>
        </div>
      </div>

      {/* 파트너 브랜드 */}
      <div style={{padding:'40px',borderTop:'1px solid #e8e8e8',borderBottom:'1px solid #e8e8e8'}}>
        ...
        <div style={{display:'flex',gap:'48px',alignItems:'center',justifyContent:'center',flexWrap:'wrap'}}>
          {['MARCIA','MATIN KIM','EENK','D.POUND','ANOTHER A','EIGHT','ANDERSSONBELL'].map(b => (
            <div key={b} style={{fontFamily:'Georgia,serif',fontSize:'18px',fontWeight:300,color:'#999',letterSpacing:'2px',cursor:'pointer'}}>
              {b}
            </div>
          ))}
        </div>
      ...
      <footer style={{background:'#111',padding:'56px 40px 32px'}}>
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr 1fr',gap:'40px',marginBottom:'48px',paddingBottom:'48px',borderBottom:'1px solid rgba(255,255,255,0.08)'}}>
          <div>

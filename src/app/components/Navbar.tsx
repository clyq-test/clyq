  ...
  return (
    <>
      <style>{`
        ...
        .lm-bg { position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:2000; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(4px); }
        ...
        .lm-inp { width:100%; padding:13px 14px; border:1px solid #e8e8e8; font-size:13px; outline:none; font-family:inherit; margin-bottom:10px; }
        ...
        .lm-btn { width:100%; padding:14px; background:#111; color:#fff; font-size:14px; font-weight:500; border:none; cursor:pointer; font-family:inherit; }
        .lm-btn:hover { background:#C94E1A; }
        .lm-kakao { width:100%; padding:13px; background:#FEE500; color:#111; font-size:13px; font-weight:600; border:none; cursor:pointer; font-family:inherit; display:flex; align-items:center; justify-content:center; gap:8px; margin-bottom:0; }
        .lm-div { display:flex; align-items:center; gap:12px; margin:14px 0; font-size:11px; color:#ccc; }
        .lm-div::before,.lm-div::after { content:''; flex:1; height:1px; background:#e8e8e8; }
        ...
        .lm-signup { text-align:center; margin-top:12px; font-size:12px; color:#666; padding-top:12px; border-top:1px solid #f0f0f0; }
        .lm-signup a { color:#C94E1A; font-weight:500; text-decoration:none; }
        .mega-wrap { position:absolute; top:100%; left:0; right:0; background:#fff; border-top:2px solid #111; border-bottom:1px solid #e8e8e8; box-shadow:0 8px 32px rgba(0,0,0,.08); z-index:400; padding:28px 40px; display:flex; gap:40px; }
        .mega-col { min-width:120px; }
        .mega-col-title { font-size:12px; font-weight:600; color:#111; letter-spacing:0.5px; margin-bottom:10px; padding-bottom:6px; border-bottom:1px solid #f0f0f0; }
        .mega-sub-link { display:block; font-size:12px; color:#666; text-decoration:none; padding:4px 0; font-weight:300; }
        .mega-sub-link:hover { color:#C94E1A; }
        .cat-link { padding:0 14px; height:42px; display:flex; align-items:center; gap:4px; font-size:12px; color:#666; text-decoration:none; font-weight:400; letter-spacing:0.3px; white-space:nowrap; border-bottom:2px solid transparent; cursor:pointer; transition:all 0.15s; background:none; border-top:none; border-left:none; border-right:none; font-family:inherit; }
        .cat-link:hover { color:#111; }
        .cat-link.active { border-bottom-color:#111; color:#111; }
        .ud-drop { position:absolute; top:calc(100% + 8px); right:0; background:#fff; border:1px solid #e8e8e8; box-shadow:0 4px 20px rgba(0,0,0,.1); min-width:160px; z-index:600; }
        .udi { display:flex; align-items:center; gap:8px; padding:12px 16px; font-size:13px; color:#333; cursor:pointer; text-decoration:none; border-bottom:1px solid #f5f5f5; }
        .udi:last-child { border-bottom:none; }
        .udi:hover { background:#fafafa; }
        .udi.red { color:#e74c3c; }
      `}</style>

      {/* 공지 바 */}
      <div style={{background:'#111',color:'rgba(255,255,255,0.6)',fontSize:'11px',textAlign:'center',padding:'8px 0',letterSpacing:'0.5px'}}>
        <strong style={{color:'#fff'}}>피팅박스 서비스</strong> — 입어보고 마음에 드는 것만 구매하세요 ·{' '}
        <a href="/community" style={{color:'#B08D57',textDecoration:'none',fontWeight:500}}>커뮤니티 활동으로 위디 적립 ›</a>
      </div>
      ...
      <nav style={{position:'sticky',top:0,zIndex:500,background:'#fff',borderBottom:'1px solid #e8e8e8'}}
        ...
        {/* 상단 바 */}
        <div className="nav-top" style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 40px',height:'58px',borderBottom:'1px solid #e8e8e8'}}>

          {/* 로고 + 메뉴 */}
          <div style={{display:'flex',alignItems:'center',gap:'28px'}}>
            <a href="/" style={{fontFamily:'Georgia,serif',fontSize:'25px',fontWeight:500,letterSpacing:'5px',color:'#111',textDecoration:'none'}}>
          ...
          {/* 검색 */}
          <div className="nav-ds" style={{display:'flex',alignItems:'center',gap:'8px',border:'1px solid #e8e8e8',padding:'8px 14px',width:'240px'}}>
            ...
            <input type="text" placeholder="브랜드, 상품명 검색" style={{border:'none',outline:'none',fontSize:'12px',width:'100%',background:'transparent',fontFamily:'inherit'}}/>
          </div>

          {/* 데스크탑 아이콘 */}
          <div className="nav-di" style={{display:'flex',alignItems:'center',gap:'16px'}}>
            <a href="/withy" style={{display:'flex',alignItems:'center',gap:'5px',padding:'6px 12px',background:'#111',color:'#fff',fontSize:'11px',textDecoration:'none',transition:'background .2s'}}
            ...
            <div onClick={() => !isLoggedIn && openLogin()}
              style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'2px',cursor:'pointer',color:'#666',fontSize:'10px'}}
              onMouseEnter={e=>(e.currentTarget.style.color='#111')}
            ...
            {isLoggedIn ? (
              <div style={{position:'relative'}}>
                <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                  ...
                  <div style={{width:'30px',height:'30px',borderRadius:'50%',background:'#111',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:500}}>
                    {userName.charAt(0)}
                  </div>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:'1px'}}>
                    <span style={{fontSize:'11px',fontWeight:500,color:'#111'}}>{userName}님</span>
          ...
          <div className="nav-hb" style={{display:'none',alignItems:'center',gap:'12px'}}>
            {isLoggedIn ? (
              ...
              <button onClick={openLogin} style={{fontSize:'11px',border:'1px solid #e8e8e8',background:'#fff',padding:'6px 12px',cursor:'pointer',fontFamily:'inherit'}}>로그인</button>
            )}
            <button onClick={() => setMenuOpen(!menuOpen)}
              ...
              <div style={{width:'22px',height:'1.5px',background:'#111',opacity:menuOpen?0:1}}/>
              <div style={{width:'22px',height:'1.5px',background:'#111',transition:'all .2s',transform:menuOpen?'rotate(-45deg) translate(4px,-4px)':'none'}}/>
            </button>
          </div>
        </div>
        ...
        {/* 카테고리 탭 */}
        <div className="nav-cats" style={{display:'flex',alignItems:'center',justifyContent:'center',height:'42px',position:'relative'}}>
          {catItems.map(item => (
            <button key={item.label}
              className={'cat-link' + (megaOpen === item.label ? ' active' : '')}
              style={{color: item.red ? '#e74c3c' : item.hot ? '#C94E1A' : '#666', fontWeight: item.hot || item.red ? 600 : 400}}
              onMouseEnter={() => item.mega ? setMegaOpen(item.label) : setMegaOpen(null)}
              onClick={() => { if (!item.mega) router.push(item.href) }}>
              {item.label}
              {item.hot && <span style={{fontSize:'8px',background:'#C94E1A',color:'#fff',padding:'1px 4px',borderRadius:'2px',fontWeight:700}}>N</span>}
              {item.mega && <span style={{fontSize:'10px'}}>▾</span>}
            </button>
          ))}
          <a href="/fitting"
            style={{padding:'0 14px',height:'42px',display:'flex',alignItems:'center',gap:'5px',fontSize:'12px',color:'#fff',textDecoration:'none',fontWeight:600,whiteSpace:'nowrap',background:'#C94E1A',marginLeft:'8px'}}>
            📦 피팅박스관
          </a>
            ...
            <div className="mega-wrap">
              {megaData['브랜드'].map((col, ci) => (
                <div key={ci} className="mega-col">
                  {col.map(b => (
                    <a key={b} href="/" className="mega-sub-link" style={{fontFamily:b==='전체 브랜드 보기'?'inherit':'Georgia,serif',fontSize:'12px',letterSpacing:b==='전체 브랜드 보기'?'0':'1px'}}>{b}</a>
                  ))}
                </div>
              ))}
            </div>
        ...
        {/* 모바일 드로어 */}
        {menuOpen && (
          <div style={{position:'fixed',top:'95px',left:0,right:0,bottom:0,background:'#fff',zIndex:999,overflowY:'auto',borderTop:'1px solid #e8e8e8'}}>
            <div style={{padding:'12px 16px',borderBottom:'1px solid #e8e8e8'}}>
              <div style={{display:'flex',alignItems:'center',gap:'8px',border:'1px solid #e8e8e8',padding:'10px 14px'}}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#999" strokeWidth="2"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.35-4.35"/></svg>
            ...
            {[
              ...
              {label:'SALE 🔴', href:'/', sub:'한정 기간 특가'},
            ].map((item, i) => (
              <a key={i} href={item.href} onClick={() => setMenuOpen(false)}
                style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',borderBottom:'1px solid #f5f5f5',textDecoration:'none',background:item.hl?'#fff5f2':'#fff'}}>
                <div>
                  <div style={{fontSize:'14px',fontWeight:item.hl?600:400,color:item.hl?'#C94E1A':'#111'}}>{item.label}</div>
                  {item.sub && <div style={{fontSize:'11px',color:'#999',marginTop:'1px'}}>{item.sub}</div>}
                </div>
            ...
            {isLoggedIn ? (
              <>
                <a href="/mypage" onClick={() => setMenuOpen(false)}
                  style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 16px',borderBottom:'1px solid #f5f5f5',textDecoration:'none'}}>
                  <div>
                    <div style={{fontSize:'14px',color:'#111'}}>마이페이지</div>
                    <div style={{fontSize:'11px',color:'#999',marginTop:'1px'}}>{userName}님 · 골드 · 위디 2,400P</div>
                  </div>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
                </a>
                <div style={{padding:'14px 16px'}}>
                  <button onClick={() => { handleLogout(); setMenuOpen(false) }}
                    style={{width:'100%',padding:'12px',border:'1px solid #e8e8e8',background:'#fff',fontSize:'13px',cursor:'pointer',fontFamily:'inherit',color:'#666'}}>
                    로그아웃
                  </button>
                </div>
              </>
              ...
              <div style={{padding:'14px 16px'}}>
                <button onClick={() => { setMenuOpen(false); openLogin() }}
                  style={{width:'100%',padding:'12px',background:'#111',color:'#fff',border:'none',fontSize:'13px',fontWeight:500,cursor:'pointer',fontFamily:'inherit'}}>
                  로그인 / 회원가입
                </button>
              </div>
            )}
          ...
          <div className="lm-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="lm-body">
              {loginTab === 'login' ? (
                ...
                <>
                  <div style={{fontSize:'13px',color:'#666',marginBottom:'20px',lineHeight:1.7}}>가입 시 입력한 이메일로 임시 비밀번호를 보내드려요.</div>
